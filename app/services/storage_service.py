...
class StorageService(ABC):
    ...
    @abstractmethod
    def excluir(self, chave: str) -> bool:
        """Remove o objeto em `chave`. Nunca levanta exceção — sempre
        "melhor esforço" na própria chamada (uma falha aqui nunca deve
        interromper por conta própria quem chamou; ver a compensação de
        upload parcial em AtendimentoPublicoService, que ignora o valor
        de retorno de propósito). Retorna True/False para quem PRECISA
        saber se a remoção teve sucesso (ver DemandaService.excluir, que
        nunca apaga um registro do banco sem confirmar que o objeto saiu
        do storage primeiro) — chamadores que não precisam saber
        simplesmente não usam o retorno."""
